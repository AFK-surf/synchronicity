...
//! by name.
//!
//! Calls llc *can* expand — a constant length within [`INLINE_BUDGET`] — are
//! left for it to turn into stores, so a small `= {0}` costs what it always
//! cost. Everything this pass does not positively recognize it leaves
//! untouched: the failure mode is llc's own diagnostic, never a miscompile.
...
/// The longest constant fill or copy left for llc to expand into stores.
///
/// llc inlines an intrinsic whose store count fits `MaxStoresPerMemset` (128
/// in the BPF backend for as long as it has existed). 64 bytes fits even as
/// single-byte stores, whatever the alignment; past that this pass does not
/// model llc's store-width arithmetic, it routes the call to the host.
const INLINE_BUDGET: u64 = 64;
...
pub(crate) fn lower_mem_intrinsics(ir: &str) -> String {
    ...
    if !helpers_called.is_empty() {
        ...
        // A program that already called a helper by name declared it; a
        // second declaration of the same symbol would not parse.
        if helpers_called.contains(&Helper::Memset) && !ir.contains("@sy_memset") {
            let _ = writeln!(out, "declare {p} @sy_memset({p}, i32, i64)");
        }
        if helpers_called.contains(&Helper::Memcpy) && !ir.contains("@sy_memcpy") {
...
fn lower_line(line: &str, fresh: &mut usize) -> Option<Lowered> {
    ...
    let suffix = name
        ...
        .or_else(|| name.strip_prefix("@llvm.memcpy."))
        .or_else(|| name.strip_prefix("@llvm.memmove."))?;
    if !suffix.starts_with("p0") {
        return None;
    }

    // (dst, src|byte, len, volatile) — anything else is not the shape this
    // pass knows, and stays.
    let arguments = split_arguments(&line[name_end..])?;
    let [dst, source_or_byte, len, _volatile] = arguments.as_slice() else {
        return None;
    };
    if !len.starts_with("i64 ") {
        return None;
    }
...
#[cfg(test)]
mod tests {
    use super::lower_mem_intrinsics;
    ...
    fn a_memset_past_the_store_budget_goes_to_the_host() {
        let lowered = lower_mem_intrinsics(BIG_MEMSET);
        assert!(
            lowered.contains(
    ...
    #[test]
    fn a_memset_llc_will_expand_into_stores_is_left_alone() {
        let ir = "  call void @llvm.memset.p0.i64(ptr align 8 %1, i8 0, i64 64, i1 false)\n";
        assert_eq!(lower_mem_intrinsics(ir), ir);
    }

    #[test]
    fn a_length_only_known_at_run_time_goes_to_the_host() {
        let ir = "  call void @llvm.memset.p0.i64(ptr %1, i8 0, i64 %n, i1 false)";
        let lowered = lower_mem_intrinsics(ir);
        assert!(
...
declare void @llvm.memset.p0.i64(ptr nocapture writeonly, i8, i64, i1 immarg) #3
  ...
  %5 = call i64 @sy_read(i64 0, ptr %1, i64 4096)";
        for line in unrecognized.lines() {
            ...
            assert_eq!(lower_mem_intrinsics(&with_newline), with_newline);
        }
    }
}
