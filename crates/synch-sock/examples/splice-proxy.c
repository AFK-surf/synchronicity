 ...
 *
 * Read `tcp-proxy.c` first for the annotated version of everything this file
 * takes as read — why the upstream is a compile-time constant, why each
 * direction has to be allowed to end on its own — and for the per-caller rate
 * limit, which that example has and this one leaves out to keep the loop bare.
 */
