...
pub enum EntryKind {
    ...
    Tombstone,
    /// **Historical.** A socket, back when a socket was a file at a path and
    /// the activation table decided the kind (`docs/SOCKETS.md` §2).
    ///
    /// Nothing emits it any more: a socket is a name of its own and only the
    /// *program* is in the tree, published as an ordinary
    /// [`File`](EntryKind::File) (`docs/SOCKET-PROGRAMS.md` §3). The variant
    /// stays so that records older builds published still decode — postcard
    /// numbers variants by position — and every reader treats it as a file
    /// with content, which is all it ever was on disk.
    Socket,
}
...
impl EntryKind {
    ...
    ///
    /// The historical [`EntryKind::Socket`] does too, which is what made an
    /// ELF an ordinary CAS object: replicated, fetched, verified per 16 KiB
    /// group, and pinnable like anything else.
