...
| Trie T6: publisher scope check | Production Lean `ScopeCheck.firstOutside`, using shared serving predicates and bounded walk | `TrieScopeCheckProofs.successful_scope_check_grants_every_entry` proves that a successful check authorizes every independently specified snapshot entry, when its raw records are available through the actual invocation and no I/O faults are injected. Missing/malformed records cannot authorize an ungranted held entry. This does not establish that the caller supplied the right grant, or prove atomic promotion. |
...
The scope check's entry-authorization theorem also passed warning-free build,
standalone kernel and standard-axiom checks. Establishing actual grant authority
and composing atomic promotion remain open.
...
| Downloading more preserves what you already have. | `CasTransferHistories.transfers_preserve_readable_content`: arbitrary finite actual receive histories preserve every initially readable range of unchanged-size file-backed content, including decoder failures and overlapping transfers, under the stated healthy-store/Bao contracts. |
| Duplicates and download order do not matter. | `CasTransferHistories.transfer_order_does_not_change_reads` and `replaying_transfers_does_not_change_reads`: identical actual range-read results after reordering or replay, for unchanged-size file-backed content and a fixed trusted decoder. Timestamps and physical interval encodings need not agree. The displaced planner-only promise wrappers are deleted. |
