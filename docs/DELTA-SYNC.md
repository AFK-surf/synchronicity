...
| An entry overstates a root's size | The row records the claim, and the next writer with a different one replaces it — a size is only settled once the final group is held, because no earlier group's chaining value depends on it. The claim can only ever be wrong within the last 16 KiB group: anything that changes the object's group count changes the shape of its tree, so no proof or slice for it would verify. A lie that does cross a group boundary names an object that can never be served: every fetch fails verification against the shape the claimed size implies, the claim never settles, and the repair is the origin republishing the entry with the object's true length. |
