...
| Identity/authority | Production Lean origin APIs and whole authority/scope reads, including scope-change invalidation, borrowed promotion transactions and materialized delegation updates. A delegation's read-only spaces are decoded and materialized in Lean and separate the serving/content scope (the whole grant) from the publication scope (read-write spaces alone) in the same authority reads. Native expiry, grant, read-only, corruption and index tests. | M5: the remaining identity lifecycle and grant-to-all-publication/serving/recovery composition. |
