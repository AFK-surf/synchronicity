...
| `Bridge` | the CAS/trie bridge: publication and promotion pair a finite sequence of per-root view steps (`ViewCellTxn`) with the head flip they share in one transaction (`ViewTxn`) | `live_leaf_flips_head` — no step stands a new source or replica leaf without an active, materialized head; `viewCellTxn_size` — materialization transactions do not change CAS size; `step_cells` exposes a free cell step, transaction trace, or unchanged cell |
