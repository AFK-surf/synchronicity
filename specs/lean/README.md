...
| `TrieServePrivacyProofs` | Scope privacy across whole node and value answers: every node payload decodes to a node admitted at a requested, admitted position; every value payload has a requested, admitted holder whose decoded node names the value and grants its key; the loop invariants cover arbitrary batches, host failures, deduplication and budget exits; shared-leaf fixtures check both request orders and entirely refused batches |
| `TrieMissingProofs` | Requesting walk used by the completeness command, with fetch integration still open: both set instances satisfy membership/erase laws; deferred holders stay outstanding and are revisited, interrupted reads retain the pending position and canonicality faults remain terminal; scope admission holds across whole batches; shared-state histories cover values, redaction, provenance, reference pairing and read failures; exhaustion/coverage remains open |
| `TrieCompleteProofs` | Whole completeness operation: a new answer is the memo's verdict when the walk is exhausted, false otherwise; certification uses the ticket read before the walk and the computed scope/owner key; histories cover cache hits, missing values, stale/blocked/terminal tickets, transactional validation without caching, and failure at every effect of a full-view operation |
| `TrieCollectProofs` | Collecting the trie: over any lawful mark set (the command's hash set and the fixtures' list both proven lawful) the walk marks every node reachable from a root it started from and every out-of-line value a marked node names, leaving the store untouched; a set-wise sweep keeps exactly the kept keys; the memo keys' byte layout, shared by every reader and the sweep; and on a concrete store the pass keeps exactly the retained root's rows, provenance and values, sweeps a store with no head whole, keeps the scoped certificate, and rolls back at every injected failure |
| `TrieWalkProofs` | Structural walks: the nibble packing a key goes through and back; a value compared as a value; over any step and any effect algebra the descent keeps every frame under its base, never charges past the ceiling (a real position at the ceiling is refused) and keeps what the step keeps; every entry a scan lists starts with the prefix, sorts after the resume cursor, and the listing is within the limit; and on a concrete trie the listing under prefix, cursor and limit, a refused position reading as empty, the diff with shared subtrees pruned unread and representations reconciled by one digest, the streamed materialization whole and scoped, with a failure injected at every effect |
| `TrieMerkleProofs` | Merkle proofs: verification is the lookup over the proof's own nodes as a raw snapshot, so a verified value is a path through those nodes and nothing else; the lookup is always answered within verification's budget; on a host that answers its digests, verifying is its pure model; on a store addressed by its digests the proof `prove` builds verifies to exactly what `get` answers (the round trip); and on a canonical trie the proofs of present and absent keys, their verification, and the refusals a truncated path, a substituted payload, a foreign root and non-node bytes earn, with a failure injected at every effect |
| `SuspensionProofs` | Suspending on a peer: `Balanced`, the discipline that a suspending effect is raised only while no transaction is open, tracked along every path of replies by a guard that classifies which effects open, close and suspend; `Suspending` composes through `pure`, `bind`, a raised effect and a transaction around a nested body; the probe command is `Suspending` outside a transaction and not inside; on the simulated host, whose peer refuses a request while a transaction is pending the way the runner does, the probe answers the counts outside and the protocol failure with the rollback inside |
