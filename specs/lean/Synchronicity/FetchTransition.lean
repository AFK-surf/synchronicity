...
theorem incomplete_settlement (origin : Origin.Parsed) (refused : List (UInt64 × ByteArray × ByteArray))
    ...
    (before : HeadView.Represents state.db view)
    (after : HeadView.Represents (execute (FetchLifecycle.settle origin refused target key result) state).2.db nextView) :
    HeadTransition [TargetTransition.fetchKey target] view nextView := by
  apply TargetTransition.refines _ before after
  · exact HeadKeyFrame.no_new_keys _
      (FetchLifecycle.settle_without_publication [] origin refused target key result notComplete (empty_unmatched target))
      (fun predicate => FetchLifecycle.effects_preserve_keys predicate []) state closed
  · intro row present different
    rw [TargetTransition.fetch_fields] at different
...
theorem completed_settlement (origin : Origin.Parsed) (refused : List (UInt64 × ByteArray × ByteArray))
    ...
    (after : HeadView.Represents (execute (FetchLifecycle.settle origin refused target key (.ok true)) state).2.db nextView) :
...
theorem settlement (origin : Origin.Parsed) (refused : List (UInt64 × ByteArray × ByteArray))
  ...
  cases result with
  | error error => exact incomplete_settlement origin refused target key _ (by simp) state closed before after
  | ok result =>
    cases result with
    | false => exact incomplete_settlement origin refused target key _ (by simp) state closed before after
    | true => exact completed_settlement origin refused target key state closed before backed after
