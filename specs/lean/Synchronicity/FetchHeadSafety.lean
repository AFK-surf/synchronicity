...
def allowed (row : Fields) (A : Type) : Fetch.Effects A → Prop
  ...
  | .left (.right (.left effect)) => ProtectedHead.accessAllowed row effect
  | _ => True
...
theorem effects_retain (row : Fields) (effect : Fetch.Effects A) (safe : allowed row _ effect)
  ...
  | left effect =>
    ...
    | right effect =>
      cases effect with
      | left effect => exact ProtectedHead.access_retains row effect safe state kept
  ...
  · apply Only.forIn
    ...
    · split
      · refine Only.seq (Only.raise _ _ trivial) fun observed => ?_
        ...
        · exact .done _
        · split
          · exact Only.seq (Only.raise _ _ (by apply Or.inl; change valueSpace ≠ "heads"; decide)) fun _ => .done _
          · exact .done _
      · refine (verify_within row hash bytes).seq fun verdict => ?_
        ...
        | accepted =>
          refine Only.seq (Only.raise _ _ (by apply Or.inl; change nodeSpace ≠ "heads"; decide)) fun _ => ?_
          cases target.context.owner with
          | none => exact .done _
          | some _ => exact Only.seq (Only.raise _ _ (by apply Or.inl; change "trie_node_origins" ≠ "heads"; decide)) fun _ => .done _
