...
pub trait Handler: Sized {
    ...
    /// Called when a forwarded streamlocal channel is requested.
    #[allow(unused_variables)]
    fn channel_open_forwarded_streamlocal(
        &mut self,
        channel: Channel<Msg>,
    ...
    /// Called when an OpenSSH agent-forwarding channel is requested.
    #[allow(unused_variables)]
    fn channel_open_agent(
        &mut self,
        channel: Channel<Msg>,
        reply: ChannelOpenHandle,
        session: &mut Session,
    ...
    /// Called for a channel type not otherwise understood by russh.
    ///
    /// `channel_type` and `payload` are the exact type name and opaque bytes
    /// following the standard channel-open fields. Dropping `reply` rejects
    /// the channel, just like the typed callbacks.
    #[allow(unused_variables)]
    fn channel_open_unknown(
        &mut self,
        channel: Channel<Msg>,
        channel_type: &str,
        payload: &[u8],
