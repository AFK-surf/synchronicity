...
impl Handle {
    ...
    /// Opens a channel with an application-defined type and opaque payload.
    pub async fn channel_open_unknown(
        &self,
        channel_type: String,
        payload: Vec<u8>,
...
impl Session {
    ...
    fn dispatch_msg(&mut self, msg: Msg) -> Result<(), Error> {
        match msg {
            ...
            Msg::ChannelOpenUnknown {
                channel_type,
                payload,
                channel_ref,
            } => {
                let id = self.channel_open_unknown(&channel_type, &payload)?;
                self.channels.insert(id, channel_ref);
            }
    ...
    /// Opens an application-defined channel with an opaque type-specific
    /// payload.
    pub fn channel_open_unknown(
        &mut self,
        channel_type: &str,
        payload: &[u8],
    ) -> Result<ChannelId, Error> {
        if channel_type.is_empty() || channel_type.len() > 256 {
            return Err(Error::Inconsistent);
        }
        self.channel_open_generic(channel_type.as_bytes(), |write| {
